...
let bullets = [], enemies = [], particles = [], rings = [], texts = [], stars = [];
let core = { x: W / 2, y: H / 2, hp: 100, radius: 34 };
...
function resetGame(){
  ...
  bullets=[]; enemies=[]; particles=[]; rings=[]; texts=[];
  core.hp=100; core.x=W/2; core.y=H/2;
...
canvas.addEventListener('mousedown', e => { if(running && e.button===0) fire(); });
...
function fire(){
  ...
  bullets.push({x:core.x+ux*42,y:core.y+uy*42,vx:ux*980,vy:uy*980,r:5,life:1.1});
  muzzle(core.x+ux*42,core.y+uy*42,ux,uy);
...
function novaBlast(){
  if(nova < .999) return;
  nova=0;
  rings.push({x:core.x,y:core.y,r:20,max:Math.min(W,H)*.64,life:.65,age:0});
  shake=18;
  let killed=0;
  enemies.forEach(e=>{
    if(dist(e,core)<Math.min(W,H)*.42){ e.dead=true; burst(e.x,e.y,e.color,22,2.3); killed++; }
  });
...
function update(dt){
  ...
  for(const e of enemies){
    ...
    if(Math.hypot(e.x-core.x,e.y-core.y)<e.r+core.radius){
      e.dead=true; core.hp-= e.type==='tank'?30: e.type==='zig'?18:12; burst(e.x,e.y,'#ff315e',25,2); shake=14; combo=1; comboTimer=0;
      if(core.hp<=0) endGame();
    }
  }
  ...
  for(const b of bullets){
    for(const e of enemies){
      ...
      if(Math.hypot(b.x-e.x,b.y-e.y)<b.r+e.r){
        b.life=0; e.hp--; spark(b.x,b.y,e.color,8);
        if(e.hp<=0){
          e.dead=true; score += e.score*combo; combo=Math.min(combo+1,99); comboTimer=2.1; burst(e.x,e.y,e.color,e.type==='tank'?34:18,e.type==='tank'?2.5:1.8); shake=Math.max(shake,e.type==='tank'?9:4);
...
function updateHUD(){
  scoreEl.textContent=Math.floor(score).toLocaleString(); waveEl.textContent=wave; comboEl.textContent=combo+'X'; novaFill.style.width=(nova*100)+'%';
}
...
function draw(){
  ...
  drawBackground(); drawRings(); drawCore(); drawBullets(); drawEnemies(); drawParticles(); drawTexts(); drawVignette();
  ctx.restore();
}

function drawBackground(){
  const g=ctx.createRadialGradient(W/2,H/2,10,W/2,H/2,Math.max(W,H)*.65); g.addColorStop(0,'#111845');g.addColorStop(.42,'#080b20');g.addColorStop(1,'#010208');ctx.fillStyle=g;ctx.fillRect(0,0,W,H);
...
function drawCore(){
  ...
  ctx.rotate(-elapsed*1.7);ctx.strokeStyle='#ff4bd6';ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,0,24,0,TAU);ctx.stroke();
  ctx.restore();
...
function drawBullets(){
  ctx.save();ctx.globalCompositeOperation='lighter';
  for(const b of bullets){ctx.strokeStyle='rgba(83,241,255,.45)';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(b.x-b.vx*.02,b.y-b.vy*.02);ctx.lineTo(b.x,b.y);ctx.stroke();ctx.fillStyle='#fff';ctx.shadowBlur=16;ctx.shadowColor='#4ff2ff';ctx.beginPath();ctx.arc(b.x,b.y,b.r,0,TAU);ctx.fill();}
  ctx.restore();
}
...
function drawRings(){
  ctx.save();ctx.globalCompositeOperation='lighter';
  for(const r of rings){const a=clamp(r.life/.65,0,1);ctx.strokeStyle=`rgba(105,245,255,${a})`;ctx.lineWidth=12*a+2;ctx.shadowBlur=40;ctx.shadowColor='#c94cff';ctx.beginPath();ctx.arc(r.x,r.y,r.r,0,TAU);ctx.stroke();}
  ctx.restore();
}

function drawTexts(){
  ctx.save();ctx.textAlign='center';ctx.textBaseline='middle';ctx.font='900 24px system-ui';
...
function burst(x,y,color,count=18,power=1.8){ for(let i=0;i<count;i++){const a=Math.random()*TAU,s=rand(90,330)*power;particles.push({x,y,vx:Math.cos(a)*s,vy:Math.sin(a)*s,life:rand(.28,.72),maxLife:.72,size:rand(3,9),color,rot:rand(0,TAU),spin:rand(-9,9)});} }
function spark(x,y,color,count=8){burst(x,y,color,count,.55);}
function muzzle(x,y,ux,uy){ for(let i=0;i<7;i++){const a=Math.atan2(uy,ux)+rand(-.45,.45),s=rand(70,220);particles.push({x,y,vx:Math.cos(a)*s,vy:Math.sin(a)*s,life:rand(.08,.22),maxLife:.22,size:rand(2,5),color:'#7ff7ff',rot:0,spin:0});} }
function flashText(text,x,y,color,size){texts.push({text,x,y,color,size,life:1.15,maxLife:1.15});}
function showBossWarning(){bossWarning.classList.remove('hidden');setTimeout(()=>bossWarning.classList.add('hidden'),1500);for(let i=0;i<3;i++) setTimeout(()=>spawnEnemy(),500+i*220);}
function endGame(){running=false;finalScore.textContent=Math.floor(score).toLocaleString();gameOver.classList.remove('hidden');}

function loop(now){
  if(!running) return;
  const dt=Math.min(.033,(now-last)/1000||.016);last=now;update(dt);draw();requestAnimationFrame(loop);
}
